...
class ChapterTest {
    ...
    static List<String> malachiChapter4Texts = List.of(
            "<여호와께서 정하신 날> 만군의 여호와가 이르노라 보라 용광로 불 같은 날이 이르리니 교만한 자와 악을 행하는 " +
            ...
            "그가 아버지의 마음을 자녀에게로 돌이키게 하고 자녀들의 마음을 그들의 아버지에게로 돌이키게 하리라 돌이키지 " +
    ...
    static void beforeAll() {
        ...
        int verseNo = 1;
        for (String texts : malachiChapter4Texts)
            verses.add(new Verse(verseNo++, List.of(texts)));
    ...
    void getVerses() {
        ...
        for (int verseNo = 0; verseNo < 6; verseNo++) {
            String text = malachiChapter4Texts.get(verseNo);
            Verse verse = verses.get(verseNo);
            assertThat(verse.getTexts().get(0)).isEqualTo(text);
        }
    }
